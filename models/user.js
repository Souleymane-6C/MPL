...
const userSchema = new Schema( 
  { 
    ...
    zipCode: {type: Number, 
      min: [10000, "Code postal trop court"],       max: 99999 
    }, 
    role: {
      type: String,
    ...
    googleId: String,
    courses: [{ type: Schema.Types.ObjectId, ref: "Course" }],     subscribedAccount: { type: Schema.Types.ObjectId, ref: "Subscriber" } 
...
userSchema.pre("save", function(next) {   let user = this; 
  ...
  bcrypt.hash(user.password, 10) 
    ...
    .catch(error => { 
      console.log(`Erreur de hachage du mot de passe: ${error.message}`);       next(error); 
    }); 
}); 
 ...
 userSchema.pre("save", function(next) {   let user = this; 
  if (user.subscribedAccount === undefined) { 
    mongoose.model("Subscriber").findOne({ email: user.email }) 
      .then(subscriber => { 
        user.subscribedAccount = subscriber;         next(); 
      }) 
      .catch(error => { 
        console.log(`Erreur lors de la connexion avec l'abonné: ${error.message}`);         next(error); 
      }); 
  } else { 
    next(); 
  } 
